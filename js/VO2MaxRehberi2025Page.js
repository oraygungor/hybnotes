...
window.VO2MaxRehberi2025Page = ({ lang = 'tr' }) => {
    ...
    const BarChart = () => (
        <div className="flex flex-col gap-3 w-full font-mono text-xs">
            {[
                { label: 'RST', val: 1.04, color: 'bg-primary', width: '100%' },
                { label: 'HIIT', val: 1.01, color: 'bg-primary/80', width: '97%' },
                { label: 'SIT', val: 0.69, color: 'bg-primary/60', width: '66%' },
                { label: 'CT', val: 0.29, color: 'bg-slate-600', width: '28%' }
            ].map((d) => (
    ...
    const RankingChart = () => (
        <div className="flex flex-col gap-3 w-full font-mono text-xs">
            {[
                { label: 'RST', val: '88%', width: '88%' },
                { label: 'HIIT', val: '85%', width: '85%' },
                { label: 'SIT', val: '51%', width: '51%' },
                { label: 'CT', val: '23%', width: '23%' }
            ].map((d, i) => (
                <div key={d.label} className="w-full">
                    <div className="flex justify-between mb-1 text-slate-300">
                        <span className={i===0 ? "text-primary font-bold" : ""}>{d.label}</span>
                        <span>{d.val}</span>
                    </div>
                    <div className="h-2 w-full bg-slate-700/50 rounded-full overflow-hidden">
                        <div className={`h-full rounded-full ${i===0 ? 'bg-primary' : 'bg-slate-500'} transition-all duration-1000 ease-out`} style={{ width: d.width, opacity: 1 - (i * 0.15) }}></div>
                    </div>
                </div>
            ))}
        </div>
    ...
    // Simple SVG Line Chart for "Sweet Spot"
    const CurveChart = ({ type }) => {
        // Points for visual representation
        const hiitPoints = "0,100 20,80 50,20 70,0 90,40 100,80"; // Inverted U
        const sitPoints = "0,5 30,5 48,5 50,5 52,90 100,95"; // Threshold drop
        const points = type === 'hiit' ? hiitPoints : sitPoints;
        const color = "currentColor"; // Will inherit text-primary
    ...
    return (
        <div className="animate-fade-in space-y-12 pb-10">
            ...
            <section>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {[
                        ...
                        <div key={card.k} className="bg-slate-800/50 backdrop-blur border border-slate-700 p-6 rounded-2xl hover:border-primary/50 transition-all hover:-translate-y-1 duration-300">
                            <div className="flex justify-between items-start mb-3">
                                <h3 className={`text-xl font-black ${i===0 ? 'text-primary' : 'text-slate-200'}`}>{card.title}</h3>
                                <span className={`text-[10px] font-bold px-2 py-1 rounded uppercase tracking-wider ${i===1 ? 'bg-amber-500/10 text-amber-400 border border-amber-500/20' : 'bg-primary/10 text-primary border border-primary/20'}`}>
                                    {card.badge}
                                </span>
                            </div>
            ...
            {/* Sweet Spots Section */}
            <section>
                <h2 className="text-2xl font-bold text-white mb-6 border-l-4 border-emerald-500 pl-4">{t.charts.sweetSpot}</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    {/* HIIT Curve */}
                    <div className="bg-slate-800 p-6 rounded-2xl border border-slate-700 relative overflow-hidden">
                        <div className="absolute top-0 right-0 w-32 h-32 bg-primary/5 rounded-full blur-2xl"></div>
                        <h3 className="text-lg font-bold text-primary mb-2">{t.charts.hiitCurve}</h3>
            ...
            {/* Protocols Tabs */}
            <section>
                <h2 className="text-2xl font-bold text-white mb-6 border-l-4 border-pink-500 pl-4">{t.sections.protocols}</h2>
            ...
            {/* Editor's Note */}
            <div className="bg-gradient-to-r from-slate-800 to-slate-900 border-l-4 border-amber-500 rounded-r-xl p-6 shadow-lg">
...
### 2. `App.js` Değişiklikleri

`App.js` dosyanızda şu değişiklikleri yapmanız gerekiyor.

**1. `VALID_PAGES` listesine yeni sayfayı ekleyin:**
```javascript
...
**2. `MENU_ITEMS` içinde `Koşu` (running) altına yeni sayfayı ekleyin:**
```javascript
// ...
{ 
    id: 'running', 
    ...
    children: [
        ...
        // YENİ EKLENEN SATIR:
        { id: 'vo2max_guide', title: lang === 'tr' ? 'VO2max Rehberi' : 'VO2max Guide', icon: Icons.Activity }, 
    ]
},
// ...
```

**3. `renderContent` fonksiyonundaki `switch` bloğuna yeni case'i ekleyin:**
```javascript
// ...
...
**4. `index.html` dosyanıza script'i ekleyin:**
```html
<script src="VO2MaxRehberi2025Page.js"></script>
